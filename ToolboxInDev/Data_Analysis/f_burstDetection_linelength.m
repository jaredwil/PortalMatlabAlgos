function [eventTimesUSec, eventChannels] = f_burstDetection_linelength(dataset,channels,params)
...
%     end
    ...
    if sum(isnan(tmpData)) ~= length(tmpData)
        %detect bursts
        [startTimesSec, endTimesSec, chan] = burstDetector(tmpData, fs, channels,params); 
