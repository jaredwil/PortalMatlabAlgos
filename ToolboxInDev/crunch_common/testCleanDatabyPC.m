function [out, weights] = testCleanDatabyPC(action, tdata);
%
% to use call:
%
% cleaned = testCleanDatabyPC(rawdata);     % to clean the data without plotting
% cleaned = testCleanDatabyPC(rawdata,0);   % to clean the data without plotting
% cleaned = testCleanDatabyPC(rawdata,1);   % to clean the data with plotting
%
...
if ischar(action)
    switch(action)
        ...
        case('GetCleanedData')
            ...
            if showData
                ptitle = 'raw';
                testCleanDatabyPC('PlotData',tdata);
            end;


           [out, weights] = testCleanDatabyPC('pca2',tdata);		% get 1st cleaned estimate
           if showData
                ptitle = 'cleaned';
                testCleanDatabyPC('PlotData',out);
            end;
...
%                pc=noti*v;               % project tdata onto v
                pc=tdata*v;               % project tdata onto v
                ...
                a1(i)=Cpc(1,end)/Cpc(1,1);
                a2(i)=Cpc(2,end)/Cpc(2,2);
                a3(i)=Cpc(3,end)/Cpc(3,3);
    ...
    [out, weights] = testCleanDatabyPC('GetCleanedData',data);
