function [spikeTimes, spikeChannels] = f_spikeDetection_threshold(snapshot, layerName, blockLenHr, channels)
...
% 'blockLenHr' 	[integer]: length (in hours) of each block to process
...
%         disp(['Found ' num2str(size(startTimesSec,1)) ' spikes']);
    ...
    disp(['Processed block ' num2str(j) ' of ' num2str(numBlocks) ' channel ' num2str(channels)]);
    j = j + 1;
end
