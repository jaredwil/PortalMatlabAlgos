function f_burst_linelength(dataset,params)
...
% 
%   dbstop in f_burst_linelength at 76
...
% baseline - could use a dc filter, or just deal with it in clustering
  ...
  % once
  annotFile = sprintf('../Output/%s-annot-%s',dataset.snapName,params.technique);
  ftxt = fopen([annotFile '.txt'],'w');
  fclose(ftxt);  % this flushes the file
  save([annotFile '.mat'],'params');

  if params.saveToDisk
    featureFile = sprintf('../Output/%s-feature-%s',dataset.snapName,params.technique);
