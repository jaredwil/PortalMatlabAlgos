function out = CleanViewData(action, tdata);
%
% to use call:
%
% cleaned = CleanViewData(rawdata);     % to clean the data without plotting
% cleaned = CleanViewData(rawdata,0);   % to clean the data without plotting
% cleaned = CleanViewData(rawdata,1);   % to clean the data with plotting
%
...
%
% CleanViewData.m will clean the data array using a principal
...
global gridsize;        % for viewing data
% to change the global values edit the values of the variables set in case('init'), 
...
xsd = 2000;  				% no of sd's that makes something a putative spike
...
analogDisplayOffset = 200;		% number of analog units by which to displace each trace
										%		when displaying
...
if showData
  ptitle = 'raw';
  CleanViewData('PlotData',tdata);
end;


%%%%%%%%%%%%%%%%% step 2: pca the tdata %%%%%%%%%%%%%%%%%%%%%%%

pcadata = CleanViewData('pca2',tdata);		% get 1st cleaned estimate
...
if showData
%  ptitle = '1st pca run';
%  CleanViewData('PlotData', pcadata);

  ptitle = 'pca1 and pca2';
  CleanViewData('PlotData',pca12);
  
  ptitle = 'noise estimate';
  CleanViewData('PlotData',noiseEst);
end
...
biglist = CleanViewData('FindBigStuff',pcadata);   
...
NoSpikesData = CleanViewData('ReplaceBigStuff',tdata);
pcadata = CleanViewData('pca2',NoSpikesData);		% get 1st cleaned estimate
pca12 = pcadata(:,[end-1 end]);		% last two columns are pca 1 and pca2
pcadata(:,[end-1 end]) = [];		% get rid of them for now
noiseEst = NoSpikesData - pcadata;	% get noise estimate
replacearray = noiseEst;
NoSpikesData = CleanViewData('ReplaceBigStuff',tdata);	% now replace spikes with noise estimate


if showData
%  ptitle = 'spikes removed';
%  CleanViewData('PlotData',NoSpikesData);
end


%%%%%%%%%%%%%%%%% step 5: do second order cleaning %%%%%%
orig = tdata;
out = CleanViewData('itpca',NoSpikesData);

if showData
  ptitle = 'final result';
  CleanViewData('PlotData',out(:,1:end-2));
end
...
plot(tdata,'color', [1 1 0.7]);
ax = axis;
ax(1) = ax(1)-10;
ax(2) = length(tdata) + 10;
axis(ax);
...
set(gca, 'yticklabel', '');
if ~isempty(gridsize)
    set(gca, 'ytick', gridsize);
else
    gridsize = get(gca, 'ytick');
end
grid on;
xa =get(gca, 'ytick');
...
for i = 1:n
  tdata(:,i) = tdata(:,i) -(i-1)*analogDisplayOffset;
  worig(:,i) = worig(:,i) -(i-1)*analogDisplayOffset;
end


plot(worig, 'color', 'red');
hold on
plot(tdata,'color', [1 1 0.8]);
ax = axis;
ax(1) = ax(1)-10;
ax(2) = length(tdata) + 10;
axis(ax);
...
for ci = 0:last-1
  if (ci+1)*ptspercut > dataLength;
	...
	cleaneddata = CleanViewData('GetCleanedData',data(ci*ptspercut+1:stop,:));
