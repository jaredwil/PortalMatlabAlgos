...
switch action
    ...
    case 'init'
        ...
        h = figure('units', 'normalized', 'position', [0.005, 0.45, 0.90, 0.45]);
        ...
        x = 1:length(decay);
        xxx = x/(6*60*24);
        plot(xxx,decay)                           % plot all 10 second bins
        drawnow
    ...
    case 'run'
        ...
        inx = find(diff(above) > 1);
        res = xxx([above(inx)+1; above(end)]);
        plot(res, y(1)+0.1, '.r', 'MarkerSize', 8);
        
        for i = 1:length(res);
            temp = starttime;
            temp(3) = temp(3)+res(i);
            fprintf('%s\n', datestr(datenum(temp)));
        end
