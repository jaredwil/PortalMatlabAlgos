function Write2BNI(data,outname,srate,stime,sdate,labels,sens,avifile)
...
elseif isempty(data) & nargin < 2, %close session
    ...
    if nbytes > OUTFILESIZE | strcmp(outname,'newfile'),
        fclose(FILEID);
        fprintf('File %s created\n', OUTFILE);
        FILEHDR.mpginfo(1).endtime = [NPTS/FILEHDR.rate NPTS 27];
        WriteBNIHdr;
...
writebnihdr(bnihdrfile,OUTFILE,FILEHDR,(1:FILEHDR.nchan)',FILEHDR.labels);
...
%  writebnihdr(bnihdrfile,bnifile,sdate,stime,rate,nchan,sens,labels,nextfile,events)
global FILEHDR
